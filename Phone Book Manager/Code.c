    ...
    if (contactCount < MAX_CONTACTS) {
        ...
        while (1) {
            ...
            if (isValidPhoneNumber(tempPhone)) {
                strcpy(contacts[contactCount].phone, tempPhone);
                break;
            } else {
...
void searchContact() {
    ...
    for (int i = 0; i < contactCount; i++) {
        ...
        if (strstr(contacts[i].name, name) != NULL) { // Partial match (e.g., "Ramesh" finds "Ramesh Kumar")
        // if (strcmp(contacts[i].name, name) == 0) { // Exact match
            printf("Name: %s, Phone: %s\n", contacts[i].name, contacts[i].phone);
            foundCount++;
        }
    }
